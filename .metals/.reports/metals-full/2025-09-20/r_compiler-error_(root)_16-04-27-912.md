error id: C3E13C755983F235B44FCA8B9557D3B1
file://<WORKSPACE>/src/main/scala/gibbon/sources/http/HttpSource.scala
### java.lang.AssertionError: assertion failed: (List(Stamp(2023-10-16T09:31:08Z,4340080,(dev=100000e,ino=80238597))),0)
...
action parameters:
uri: file://<WORKSPACE>/src/main/scala/gibbon/sources/http/HttpSource.scala
text:
```scala
        ...
        response.status match {
          case StatusCodes.OK =>
            response.entity.dataBytes
              .map(_.utf8String)
              .map(parser)
              .runFold(Seq.empty[Event[K, V]])(_ :+ _)
              .map(events => AkkaSource(events))
          case status =>
            throw new RuntimeException(s"HTTP request failed with status: $status")
        }
      }
    }
  }
